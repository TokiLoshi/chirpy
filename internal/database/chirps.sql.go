...
const getAuthoredChirps = `-- name: GetAuthoredChirps :many
SELECT id, created_at, updated_at, body, user_id FROM chirps WHERE user_id=$1
`

func (q *Queries) GetAuthoredChirps(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAuthoredChirps, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
